...
#let project(
  title: "", familyname: [], firstname: [], phone: "", email: "", github: "", left_column_size: left_column_size, grid_column_gutter: grid_column_gutter, main_color: main_color, heading_color: heading_color, firstname_color: firstname_color, familyname_color: familyname_color, body,
) = {
  let author = firstname + familyname
  set document(author: author, title: title)
  set page(
    numbering: "1/1", margin: (top: 1.5cm, left: 2cm, right: 2cm), footer: [
   ...
   * - h1: section (colored, prominent, with colored rectangle, spans two
   *   columns)
   * - h2: role (bold)
   * - h3: place (italic)
  ...
  set list(
    marker: box(circle(radius: 0.2em, stroke: heading_color), inset: (top: 0.15em)),
  )
  ...
  align(
    ...
    // Author information.
    #text(
      [#text(firstname, fill: firstname_color) #text(familyname, fill: familyname_color)], weight: 400, 3.4em,
    )
    ...
    // Title row.
    #block(
      text(weight: 400, 1.5em, title, style: "italic", fill: familyname_color),
    )
  ]
...
#let cvline(left: [], text: []) = cvgrid(align(center, left), text)

#let textit(content) = (text(content, style: "italic"))

// Link Definitions:

#let iconlink(icon: "", url: "", linktext: []) = [
  #box[
    #pad(right: 0.5em)[
...
#let sourcelink(url) = iconlink(icon: "git-alt-brands-solid", url: url, linktext: "Source")

#let projectlink(url) = iconlink(icon: "file-lines-solid", url: url, linktext: "Project")

#let paperlink(url) = iconlink(icon: "file-pdf-solid", url: url, linktext: "Paper")

#let cvpub(title: "", authors: [], venue: "", icons: [], project: "", paper: "") = cvgrid(align(center, []), [
  == #title
...
#let cvproject(title: "", git: "", description) = cvgrid([], [
  == #title
  #description
